...
namespace SeqLib
{
    ...
    public:
        ...
        {
            if (m_bams.size())
                return m_bams.begin()->second.fp.get() != NULL;
            return false;
        }
        ...
        Return a shared pointer to the raw htsFile object
          @exception Throws runtime_error if the requested file has not been opened
        already with Open
        ...
        Return a shared pointer to the raw htsFile object from the first BAM
          @exception Throws runtime_error if no files have been opened already with
        Open
          @param f File to retrieve the htsFile from.
        ...
        */
        ...
        /** Reset the given BAM/SAM/CRAM to the begining, but keep the loaded indicies
         * and file-pointers
         * @param f Name of file to reset
         * @return Returns false if this BAM is not found in object
         * @note Unlike Reset(), this version will NOT reset the regions, since other
         * BAMs may still be using them.
         */
