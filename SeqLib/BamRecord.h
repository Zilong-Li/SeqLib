...
static const char BASES[16] = {' ', 'A', 'C', ' ', 'G', ' ', ' ', ' ', 'T', ' ', ' ', ' ', ' ', ' ', ' ', 'N'};

static std::string cigar_delimiters = "MIDNSHPX";

static const uint8_t CIGTAB[255] = {
...
namespace SeqLib
{
    ...
    public:
        ...
        {
            return bam_cigar_opchr(data);
        }

        /** Return the raw sam.h uint8_t cigar type (bam_cigar_op(data)) */
        inline uint8_t RawType() const
        {
            return bam_cigar_op(data);
        }

        /** Return the length of the cigar op (eg 35M returns 35) */
        inline uint32_t Length() const
        {
            return bam_cigar_oplen(data);
        }

        /** Returns true if cigar op matches bases on the reference (MDN=X) */
        inline bool ConsumesReference() const
        {
            return bam_cigar_type(bam_cigar_op(data)) & 2;
        }

        /** Returuns true cigar op matches bases on the query (MIS=X) */
        inline bool ConsumesQuery() const
        {
            return bam_cigar_type(bam_cigar_op(data)) & 1;
        }

        /** Return whether two CigarField objects have same op and len */
        inline bool operator==(const CigarField& c) const
        {
            return c.Type() == Type() && c.Length() == Length();
        }
    ...
    public:
        ...
        {
            return m_data.begin();
        } ///< Iterator (aka std::vector<CigarField>.begin()
        iterator end()
        {
            return m_data.end();
        } ///< Iterator (aka std::vector<CigarField>.end()
        ...
        inline const CigarField& back() const
        {
            return m_data.back();
        }

        /** Reference to last cigar op */
        inline CigarField& back()
        {
            return m_data.back();
        }

        /** Const reference to first cigar op */
        inline const CigarField& front() const
        {
            return m_data.front();
        }

        /** Reference to first cigar op */
        inline CigarField& front()
        {
            return m_data.front();
        }
        ...
        inline CigarField& operator[](size_t i)
        {
            return m_data[i];
        }

        /** Returns the i'th cigar op (const) */
        const CigarField& operator[](size_t i) const
        {
            return m_data[i];
        }
    ...
    public:
        ...
        inline bool ReverseFlag() const
        {
            return b ? ((b->core.flag & BAM_FREVERSE) != 0) : false;
        }

        /** BamRecord has mate aligned on reverse strand */
        inline bool MateReverseFlag() const
        {
            return b ? ((b->core.flag & BAM_FMREVERSE) != 0) : false;
        }

        /** BamRecord has is an interchromosomal alignment */
        inline bool Interchromosomal() const
        {
            return b ? b->core.tid != b->core.mtid && PairMappedFlag() : false;
        }

        /** BamRecord is a duplicate */
        inline bool DuplicateFlag() const
        {
            return b ? ((b->core.flag & BAM_FDUP) != 0) : false;
        }

        /** BamRecord is a secondary alignment */
        inline bool SecondaryFlag() const
        {
            return b ? ((b->core.flag & BAM_FSECONDARY) != 0) : false;
        }

        /** BamRecord is paired */
        inline bool PairedFlag() const
        {
            return b ? ((b->core.flag & BAM_FPAIRED) != 0) : false;
        }
        ...
        inline bool QCFailFlag() const
        {
            return b ? ((b->core.flag & BAM_FQCFAIL) != 0) : false;
        }

        /** BamRecord is supplementary alignment */
        inline bool SupplementaryFlag() const
        {
            return b ? ((b->core.flag & BAM_FSUPPLEMENTARY) != 0) : false;
        }

        /** BamRecord is mapped */
        inline bool MappedFlag() const
        {
            return b ? ((b->core.flag & BAM_FUNMAP) == 0) : false;
        }

        /** BamRecord mate is mapped */
        inline bool MateMappedFlag() const
        {
            return b ? ((b->core.flag & BAM_FMUNMAP) == 0) : false;
        }

        /** BamRecord is mapped and mate is mapped and in pair */
        inline bool PairMappedFlag() const
        {
            return b ? (!(b->core.flag & BAM_FMUNMAP) && !(b->core.flag & BAM_FUNMAP) && (b->core.flag & BAM_FPAIRED)) : false;
        }

        /** BamRecord is mapped in proper pair */
        inline bool ProperPair() const
        {
        ...
        inline int32_t MatePosition() const
        {
            return b ? b->core.mpos : -1;
        }
        ...
        inline int32_t ChrID() const
        {
            return b ? b->core.tid : -1;
        }

        /** Get the chrosome ID of the mate read */
        inline int32_t MateChrID() const
        {
            return b ? b->core.mtid : -1;
        }

        /** Get the mapping quality */
        inline int32_t MapQuality() const
        {
        ...
        inline void SetMapQuality(int32_t m)
        {
            if (b)
                b->core.qual = m;
        }
        ...
        /** Set the chr id */
        inline void SetChrID(int32_t i)
        {
            b->core.tid = i;
        }

        /** Set the chr id of mate */
        inline void SetChrIDMate(int32_t i)
        {
            b->core.mtid = i;
        }

        /** Set the position of the mate read */
        inline void SetPositionMate(int32_t i)
        {
            b->core.mpos = i;
        }
        ...
        inline int NumAlignedBases() const
        {
            ...
            for (size_t i = 0; i < b->core.n_cigar; i++)
                if (bam_cigar_opchr(c[i]) == 'M' || bam_cigar_opchr(c[i]) == 'I' || bam_cigar_opchr(c[i]) == '=' || bam_cigar_opchr(c[i]) == 'X' ||
        ...
        SeqPointer<bam1_t> shared_pointer() const
        {
            return b;
        }
