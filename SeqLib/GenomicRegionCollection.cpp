...
namespace SeqLib
{
    ...
    bool GenomicRegionCollection<T>::ReadVCF(const std::string& file, const BamHeader& hdr)
    {
        ...
        while (1)
        {
            ...
            {
                std::cerr << "...Could not parse pos: " << pos << std::endl << std::endl << "...on line " << line << std::endl;
            }
            if (gr.chr >= 0)
                m_grv->push_back(gr);
        }
    ...
    void GenomicRegionCollection<T>::MergeOverlappingIntervals()
    {
        ...
        for (typename std::list<T>::iterator i(intervals.begin()), iend(intervals.end()); inext != iend;)
        {
            ...
            {
                if (i->pos2 >= inext->pos2)
                    intervals.erase(inext++);
    ...
    template <class T>
    template <class K>
    GenomicRegionCollection<GenomicRegion> GenomicRegionCollection<T>::FindOverlaps(const GenomicRegionCollection<K>& subject, std::vector<int32_t>& query_id,
                                                                                    std::vector<int32_t>& subject_id, bool ignore_strand) const
    {
            ...
            if (ff != m_tree->end())
            {
                ...
                for (GenomicIntervalVector::const_iterator j = giv.begin(); j != giv.end(); ++j)
                {
                    ...
                    if (ignore_strand || (subject.at(j->value).strand == m_grv->at(i).strand))
                    {
                        ...
                        std::cerr << "find overlaps hit " << j->start << " " << j->stop << " -- " << j->value << std::endl;
#endif
                        output.add(GenomicRegion(m_grv->at(i).chr, std::max(static_cast<int32_t>(j->start), m_grv->at(i).pos1),
                                                 std::min(static_cast<int32_t>(j->stop), m_grv->at(i).pos2)));
                    }
                }
            }
        }
