...
#define DEBUGIV(msg, read)                                                                                                                                     \
    if (read.Qname() == QNAME && (read.AlignmentFlag() == QFLAG || QFLAG == -1))                                                                               \
    {                                                                                                                                                          \
        std::cerr << (msg) << " read " << r << std::endl;                                                                                                      \
    }
#else
#define DEBUGIV(msg, read)
#endif
...
namespace SeqLib
{
    ...
    namespace Filter
    {
        ...
        {
            return read_group.empty() && ins.isEvery() && del.isEvery() && isize.isEvery() && mapq.isEvery() && len.isEvery() && clip.isEvery() &&
                   nm.isEvery() && nbases.isEvery() && fr.isEvery() && (subsam_frac >= 1) && xp.isEvery()
#ifdef HAVE_C11
                   && !aho.count
#endif
                ;
        ...
        bool ReadFilter::isValid(const BamRecord& r)
        {
            ...
            for (std::vector<AbstractRule>::iterator it = m_abstract_rules.begin(); it != m_abstract_rules.end(); ++it)
                if (it->isValid(r))
                {
        ...
        ReadFilterCollection::ReadFilterCollection(const std::string& script, const BamHeader& hdr) : m_count(0), m_count_seen(0)
        {
            ...
            JSONCPP_STRING err;
            if (!reader->parse(tscript.c_str(), tscript.c_str() + rawJsonLength, &root, &err))
            {
        ...
        void FlagRule::parseJson(const Json::Value& value)
        {
            ...
            if (dup.parseJson(value, "duplicate"))
                every = false;
            if (supp.parseJson(value, "supplementary"))
                every = false;
            if (qcfail.parseJson(value, "qcfail"))
                every = false;
            if (hardclip.parseJson(value, "hardclip"))
                every = false;
            if (fwd_strand.parseJson(value, "fwd_strand"))
                every = false;
            if (mate_rev_strand.parseJson(value, "mate_rev"))
                every = false;
            if (mate_fwd_strand.parseJson(value, "mate_fwd"))
                every = false;
            if (mate_mapped.parseJson(value, "mate_mapped"))
                every = false;
            if (mapped.parseJson(value, "mapped"))
                every = false;
            if (ff.parseJson(value, "ff"))
                every = false;
            if (fr.parseJson(value, "fr"))
                every = false;
            if (rf.parseJson(value, "rf"))
                every = false;
            if (rr.parseJson(value, "rr"))
                every = false;
            if (ic.parseJson(value, "ic"))
                every = false;
        }
        ...
        bool FlagRule::isValid(const BamRecord& r)
        {
            ...
            if (!hardclip.isNA())
            { // check that we want to chuck hard clip
                if (r.CigarSize() > 1)
                {
